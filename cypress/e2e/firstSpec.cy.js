describe('Checking QA job offers in Sii career site', () => {
    ...
    it.skip('Looking for QA job offers in Sii Bydgoszcz', () => {
        ...
        cy.get('#sii-m-nav-menu__item--20392')
            .click()
        // cy.get('.sii-m-icons-menu__item__label').contains('Oferty pracy').click({ force: true})
        cy.get('.sii-m-icons-menu__item__label').contains('Oferty pracy').click()
        cy.get('.sii-m-icons-menu__item__label').contains('Oferty pracy').then (item1 => {
        ...
        cy.get('.js-ajax-load-number')
            ...
            .then((numberOfOffers) => {
                cy.log(numberOfOffers.split(': ')[1])
                cy.should("be.gte")
                // return numberOfOffers.split(': ')[1]
                // expect(parseInt.split(':')[1]).to.be.greaterThan(0)
            })
            .then(parseInt)
            .should('be.a', 'number')
    })
